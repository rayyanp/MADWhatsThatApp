 ...
 fetchChatData = async () => {
  ...
  try {
    const response = await fetch(
      `http://localhost:3333/api/1.0.0/chat/${chatId}?limit=20&offset=${offset}`,
      {
        headers: {
...
render() {
const { chatData, textMessage, isLoading, isEditing, editMessageId, editTextMessage, error, offset } = this.state;
const { chatId } = this.props.route.params;
...
const showButton = offset === 0 && chatData.messages.length >= 20;
